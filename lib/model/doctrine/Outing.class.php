...
class Outing extends BaseOuting
{
    ...
    public static function buildOutingListCriteria(&$criteria, &$params_list, $is_module = false, $mid = 'm.id')
    {
        ...
        if (!$has_id)
        {
            ...
            self::buildConditionItem($conditions, $values, $joins, $params_list, 'Compare', $m . '.up_snow_elevation', 'ousnow', $join);
            self::buildConditionItem($conditions, $values, $joins, $params_list, 'Compare', $m . '.down_snow_elevation', 'odsnow', $join);
            self::buildConditionItem($conditions, $values, $joins, $params_list, 'Relative', array($m2 . 'up_snow_elevation', $m2 . 'down_snow_elevation'), 'oudsnow', $join);
            self::buildConditionItem($conditions, $values, $joins, $params_list, 'Compare', $m . '.access_elevation', 'oaccess', $join);
            self::buildConditionItem($conditions, $values, $joins, $params_list, 'Relative', array($m2 . 'up_snow_elevation', $m2 . 'access_elevation'), 'ousacc', $join);
            self::buildConditionItem($conditions, $values, $joins, $params_list, 'Relative', array($m2 . 'down_snow_elevation', $m2 . 'access_elevation'), 'odsacc', $join);
    ...
    public static function getSortField($orderby, $mi = 'mi')
    {
        switch ($orderby)
        {
            ...
            case 'usnow': return 'm.up_snow_elevation';
            case 'dsnow': return 'm.down_snow_elevation';
            case 'access': return 'm.access_elevation';
            case 'olen': return 'm.outing_length';
            case 'cond': return 'm.conditions_status';
            case 'freq': return 'm.frequentation_status';
            case 'park': return 'm.access_status';
            case 'glac': return 'm.glacier_status';
            case 'track': return 'm.track_status';
            case 'hut':  return 'm.hut_status';
            case 'lift': return 'm.lift_status';
